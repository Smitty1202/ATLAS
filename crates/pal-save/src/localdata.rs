...
pub fn read_local_data(blob: &[u8]) -> Result<LocalData, SaveError> {
    ...
    while let Some((name, type_name, size)) = gvas::read_tag(&mut r)? {
        if name == "SaveData" && type_name == "StructProperty" {
            ...
            r.skip(16)?; // struct id
            r.optional_guid()?;
            let mut data = parse_local_save_data(&mut r)?;
    ...
    Err(SaveError::Gvas(
        "SaveData not found in LocalData save".into(),
    ))
}
...
fn scan_custom_markers(blob: &[u8]) -> Vec<CustomMarker> {
    ...
    loop {
        ...
        let Some(vector_at) = find_bytes(blob, VECTOR_NAME, location_at + LOCATION_NAME.len())
        else {
            break;
        };
        ...
        let Some(x) = read_f64_le(blob, xyz_at) else {
            break;
        };
        let Some(y) = read_f64_le(blob, xyz_at + 8) else {
            break;
        };
        let Some(_z) = read_f64_le(blob, xyz_at + 16) else {
            break;
        };
...
mod tests {
    ...
    fn coop_player_towers_defeated_grass() {
        ...
        for k in &rec.towers_defeated {
            ...
            assert!(
                rec.areas_found.contains(k),
                "tower key {k} missing from areas_found"
            );
        }
    }
}
